...
int main(int argc, char* argv[]){
    ...
    big_bucket_length = (rank == 0) ? (input_length/size + input_length % size) : (input_length / size);
    ...
    MPI_Scatter(input_buffer + (input_length%size), input_length/size, MPI_ELEM_T,
                big_bucket, input_length/size, MPI_ELEM_T, MAIN_RANK, MPI_COMM_WORLD);
    ...
    elem_t * small_buckets = (elem_t*)malloc(big_bucket_length * size * sizeof(elem_t));
    ...
    MPI_Request * small_bucket_reqs = (MPI_Request*)malloc(2*(size-1)*sizeof(MPI_Request));
    ...
    for (i=0; (int)i < size; i++){
        if ((int)i not_eq rank){ /*don't send to self*/
            MPI_Isend(small_bucket_lengths+i, 1, MPI_SIZE_T, i, LENGTH_TAG,
                      MPI_COMM_WORLD, small_bucket_reqs + small_bucket_reqs_ix++);
            MPI_Irecv(incoming_lengths + i, 1, MPI_SIZE_T, i, LENGTH_TAG,
                      MPI_COMM_WORLD, small_bucket_reqs + small_bucket_reqs_ix++);
        }
    }

    for (i=0; (int)i < 2 * (size-1); i++){
        MPI_Waitany(2 * (size-1), small_bucket_reqs, (int*)&j, MPI_STATUS_IGNORE);
    }
    ...
    for (i=0; (int)i < size; i++){
        if ((int)i not_eq rank){
            MPI_Isend(small_buckets + i * big_bucket_length,
                      ...
                      MPI_COMM_WORLD, small_bucket_reqs + small_bucket_reqs_ix++);
            MPI_Irecv(to_sort + recv_ix, incoming_lengths[i], MPI_ELEM_T, i, DATA_TAG,
                      MPI_COMM_WORLD, small_bucket_reqs + small_bucket_reqs_ix++);
        } else {
...
static inline void split_small_buckets(size_t* small_bucket_lengths, elem_t* small_buckets){
    size_t i, j;
    const ssize_t small_bucket_range = ((ssize_t)max_value - (ssize_t)min_value)/(ssize_t)size;

    for (i=0; i < big_bucket_length; i++){
        for (j=0; (int)j < size; j++){
            if ((ssize_t)big_bucket[i] <= ((ssize_t)min_value + ((ssize_t)j+1) * small_bucket_range)){
                small_buckets[j*big_bucket_length + small_bucket_lengths[j]++] = big_bucket[i];
                break;
...
static inline void output_sorted(size_t to_sort_length, elem_t* to_sort){
    ...
    if (rank == MAIN_RANK){
        ...
        MPI_Recv(&i, 1, MPI_INT, rank-1, SIGNAL_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
